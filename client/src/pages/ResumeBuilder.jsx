...
import { Check, ArrowLeftIcon, Briefcase, ChevronLeft, ChevronRight, FileText, GraduationCap, Sparkles, User, Share2Icon, EyeIcon, EyeOffIcon, DownloadIcon } from 'lucide-react'
...
const ResumeBuilder = () => {
  ...
  const sections = [
    ...
    { id: "projects", name: "Projects", icon: GraduationCap },
  ...
  await saveResume();        // save latest changes

  setTimeout(() => {
    window.print();          // wait for re-render
  }, 400);                   // small delay fixes stale print
}
  ...
  const saveResume = async () => {
    try {
      ...
      //remove image from updatedResumeData //add
      ...
      <div className='max-w-7xl mx-auto px-4 pb-8'>
        <div className='grid lg:grid-cols-12 gap-8'>
            ...
            <div className='bg-white rounded-lg shadow-sm border border-gray-200 p-6 pt-1'>
              ...
              <div className='flex justify-between items-center mb-6 border-b border-gray-300 py-1'>
                <div className='flex items-center gap-2'>
                  <TemplateSelector selectedTemplate={resumeData?.template || "classic"}
                    onChange={(template) =>
                      setResumeData(prev => ({ ...prev, template: template }))
                    }
                  />
                  <ColorPicker selectedColor={resumeData?.accent_color || "#3B82F6"} onChange={(color) => setResumeData(prev => ({ ...prev, accent_color: color }))} />
                </div>
              ...
              <div className='space-y-6'>
                {activeSection
                  .id === 'personal' && (
                    <PersonalInfoForm data={resumeData?.personal_info || {}} onChange={(updatedPersonalInfo) => setResumeData(prev => ({ ...prev, personal_info: updatedPersonalInfo }))} removeBackground={removeBackground} setRemoveBackground={setRemoveBackground} />
                  )}
                {activeSection.id === 'summary' && (
                  <ProfessionalSummaryForm data={resumeData?.professional_summary || ""} onChange={(data) => setResumeData(prev => ({ ...prev, professional_summary: data }))} setResumeData={setResumeData} />
                )}
                {activeSection.id === 'experience' && (
                  <ExperienceFrom data={resumeData?.experience || []} onChange={(data) => setResumeData(prev => ({ ...prev, experience: data }))} />
                )}
          ...
          {/* <div className='lg:col-span-7 max-lg:mt-6'>
            <div>
              <div className='relative w-full'>
                <div className='absolute bottom-3 left-3 right-0 flex items-center justify-end gap-2'>
                  ...
                  <button onClick={changeResumeVisibilty}
                  ...
                  <button onClick={downloadResume} className='flex items-center p-2 px-4 gap-2 text-xs bg-gradient-to-br from-green-100 to-green-100 to-green-200 text-green-600 ring-green-300 rounded-lg hover:ring transition-colors'>
            ...
            {/* resume preview */}
          {/* <ResumePreview data={resumeData} template={resumeData.template} accentColor={resumeData.accet_color} />
          </div> */}
          <div className='lg:col-span-7 max-lg:mt-6'>
            <div>
              <div className='relative w-full'>
